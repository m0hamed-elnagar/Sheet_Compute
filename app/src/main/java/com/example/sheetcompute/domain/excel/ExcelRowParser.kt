...
object ExcelRowParser {
    ...
    fun parseRow(
        ...
        val employee = EmployeeEntity(id, name)
                    ...
                    val clockIn = when {
                        // Universal approach that works for all POI versions
                        timeCell.cellTypeEnum == CellType.NUMERIC -> {
                        ...
                        timeCell.cellTypeEnum == CellType.STRING -> {
                            Log.d("ExcelImport", "Parsing string time value: $timeStr")
                            val timeStr = timeCell.stringCellValue.trim()
                            parseTimeString(timeStr) ?: return null
                        }

                        else -> {
                            Log.d("ExcelImport", "Parsing cell value as string: $timeStr")
